...
export default function Sidebar() {
    ...
    return (
        <div className="h-screen w-20 lg:w-64 bg-primary border-r border-white/5 flex flex-col transition-all duration-300 fixed left-0 top-0 z-40">
            ...
            <button 
                onClick={() => navigate('/')}
                className="h-20 flex items-center justify-center lg:justify-start lg:px-8 border-b border-white/5 hover:bg-white/5 transition-colors cursor-pointer"
            >
            ...
            <div className="flex-1 py-8 px-3 lg:px-4 space-y-2">
                {menuItems.map((item) => (
                    ...
                    >
                        ...
                        <span className="hidden lg:block ml-4 text-lg">{item.label}</span>
                    </button>
                ))}
            </div>
